...
def	userBasedRecommendations(ratings, wantedPredictions, similarity):
	...
	file = open('user.txt', 'a') # create a file
	ranks = {}    # store user's rank to other user's item
	#for tuple in wantedPredictions: ["user","movie"]
	user = wantedPredictions
	# movieAsked = tuple[1]
	total = {}
	similaritySums = {}
	use_dic ={}
	rec = {}
	k= 15 # k is the number of items recommand for user


	# store top N users who are same as user
	for second_person in ratings:
		if second_person == user: continue
		s = similarity(ratings,user, second_person)
		if s <= 0: continue
		use_dic[second_person] = s
	# get the most same user
	temp = sorted(use_dic.items(),key = lambda item:item[1],reverse=True)
	most_user = temp[0][0]
	s = temp[0][1]

	# get similar items in most same user
	for item in ratings[most_user]:
		if item not in ratings[user] or ratings[user][item] == 0:

			total.setdefault(item, 0)
			total[item] += int(ratings[most_user][item])*s
			similaritySums.setdefault(item, 0)
			similaritySums[item] += s
			ranks[item] = total[item]/similaritySums[item]
			rec[item] = ranks[item]
	# get first k items
	result = sorted(rec.items(),key = lambda item:item[1],reverse=True)[0:k]
	for i in range(len(result)):
		file.write("We recommand :")
...
for u in users:
	...
	if(u_prev == userID):
		...
		u_prev = userID
#userBasedRecommendations(userRatings, toBeRatedList, compute_cosine_similarity)
#userBasedRecommendations(userRatings, [["3374","673"],["5953","3082"]], sim_pearson)
final = userBasedRecommendations(userRatings, "5953", compute_cosine_similarity)
# print final
...
for m in movie_list:
    movie_dic[m[0]] = m[1]
print "TOP 15 Movies for user 5953:"
for i in range(len(final)):
    if movie_dic.has_key(final[i][0]):
        #fin.append(movie_dic[rec[i][0]])
        print movie_dic[final[i][0]]